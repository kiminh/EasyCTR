...
class _BinaryLogisticHeadWithSigmoidCrossEntropyLoss(_Head):
  ...
  def create_loss(self, features, mode, logits, labels):
    ...
    logits_list = map(ops.convert_to_tensor, logits_list)

    labels = _check_dense_labels_match_logits_and_reshape(
        labels=labels, logits=logits_list[0], expected_labels_dimension=1)
    ...
    for idx, logits in enumerate(logits_list):
      with ops.name_scope('loss_{}'.format(idx)):
        if self._loss_fn:
          unweighted_loss = _call_loss_fn(
  ...
  def _create_tpu_estimator_spec(
    ...
    with ops.name_scope(self._name, 'head'):
      ...
      (training_loss, unreduced_loss, weights, processed_labels) = (
          self.create_loss(
              features=features, mode=mode, logits=logits_list, labels=labels))
      if regularization_losses:
        regularization_loss = math_ops.add_n(regularization_losses)
        regularized_training_loss = math_ops.add_n(
            [training_loss, regularization_loss])
    ...
    with ops.name_scope(''):
      ...
      # (zhezhaoxu) add training metric summary
      metric_ops = self._eval_metric_ops(
          processed_labels, logits, logistic, class_ids,
          weights, unreduced_loss, regularization_loss)
      for key in metric_ops:
          summary.scalar(key, metric_ops[key][1])
